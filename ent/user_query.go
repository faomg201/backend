...
// QueryUSERRECORD chains the current query on the USER_RECORD edge.
...
// the "USER_RECORD" edge. The optional arguments used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUSERRECORD; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_user_record
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_record" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_record" returned %v for node %v`, *fk, n.ID)
			}
