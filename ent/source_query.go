...
// the "SOURCE_RECORD" edge. The optional arguments used to configure the query builder of the edge.
...
func (sq *SourceQuery) sqlAll(ctx context.Context) ([]*Source, error) {
	...
	if query := sq.withSOURCERECORD; query != nil {
		...
		for _, n := range neighbors {
			fk := n.source_source_record
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "source_source_record" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "source_source_record" returned %v for node %v`, *fk, n.ID)
			}
